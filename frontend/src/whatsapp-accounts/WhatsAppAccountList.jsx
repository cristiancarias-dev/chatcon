  ...
  return (
    <div className="p-4 lg:p-6">
      <div className="mb-4 flex animate-slide-up flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold text-gray-900">WhatsApp Accounts</h1>
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                ...
                <th className="px-4 py-2.5 text-xs font-semibold uppercase tracking-wider text-gray-500">Phone Number</th>
                <th className="px-4 py-2.5 text-xs font-semibold uppercase tracking-wider text-gray-500">WABA ID</th>
                <th className="px-4 py-2.5 text-xs font-semibold uppercase tracking-wider text-gray-500">Token</th>
                ...
                >
                  <td className="whitespace-nowrap px-4 py-3">
                    <div className="flex items-center gap-2.5">
                      <div className="avatar h-8 w-8 rounded-lg bg-gradient-to-br from-emerald-400 to-emerald-600 text-[10px]">
                        {acc.name.charAt(0).toUpperCase()}
                      </div>
                      <span className="font-medium text-gray-900">{acc.name}</span>
                    </div>
                  </td>
                  <td className="whitespace-nowrap px-4 py-3 font-mono text-gray-700">{acc.phone_number}</td>
                  <td className="whitespace-nowrap px-4 py-3 font-mono text-xs text-gray-500">{acc.business_account_id || "-"}</td>
                  <td className="whitespace-nowrap px-4 py-3 font-mono text-xs text-gray-400">{acc.access_token_preview}</td>
                  <td className="whitespace-nowrap px-4 py-3">
                ...
                <tr>
                  <td colSpan={isAdmin ? 6 : 5} className="px-4 py-12 text-center">
                    <div className="mx-auto max-w-sm">
                      ...
                      <p className="mt-2 text-sm font-medium text-gray-900">No WhatsApp accounts</p>
                      <p className="mt-1 text-sm text-gray-500">Add a WhatsApp Business account to start sending messages.</p>
                      {isAdmin && (
                        <Link to="/whatsapp-accounts/new" className="btn-primary mt-3">
                          Add Account
                        </Link>
                      )}
                    </div>
