...
import { getTemplates, refreshTemplates, deleteTemplate, getAccount } from "./useWhatsAppAccounts";
...
export default function WhatsAppTemplateList() {
  ...
  async function load() {
    try {
      setError("");
      setLoading(true);
      const [acct, tmpls] = await Promise.all([
        getAccount(id),
        getTemplates(id),
      ]);
      setAccount(acct);
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                ...
                <th className="px-6 py-3.5 text-xs font-semibold uppercase tracking-wider text-gray-500">Language</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {templates.map((t) => (
                <tr key={t.id} className="transition-colors hover:bg-gray-50">
                  <td className="whitespace-nowrap px-6 py-4 font-medium text-gray-900">{t.name}</td>
